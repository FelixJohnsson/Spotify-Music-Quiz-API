...
const get_room = (id) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        room_model.find({ id: id }, (error, success) => {
            if (error)
                reject(error);
            if (success)
                resolve(success);
...
const delete_room = (id) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        room_model.deleteOne({ id: id }, (error, success) => {
            if (error)
                reject(error);
            if (success)
                resolve(success);
...
const add_player = (id, new_player_object) => {
    return new Promise((resolve, reject) => {
        let update = { $push: { players: new_player_object } };
        room_model.findOneAndUpdate({ id: id }, update, { useFindAndModify: false, returnOriginal: false }, (error, success) => {
            if (error)
                reject(error);
            if (success)
                resolve([success]);
...
const remove_player = (room_id, id) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        room_model.findOneAndUpdate(filter, update, { useFindAndModify: false, returnOriginal: false }, (error, success) => {
            if (error)
                reject(error);
            if (success)
                resolve([success]);
...
const update_room = (id, type, value) => {
    switch (type) {
        case 'Increment room':
            return new Promise((resolve, reject) => {
                get_room(id)
